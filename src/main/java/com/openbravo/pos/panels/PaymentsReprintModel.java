...
public class PaymentsReprintModel {
    ...
    private Double m_dPaymentsDebt;
    private Double m_dPaymentsDebtPaid;
    ...
    public static PaymentsReprintModel emptyInstance() {
        ...
        p.m_dPaymentsDebt = 0.0;
        p.m_dPaymentsDebtPaid = 0.0;
    ...
    public static PaymentsReprintModel loadInstance(AppView app) throws BasicException {
        ...
        if (ccash == null) {
            ...
            , "SELECT COUNT(*), SUM(payments.TOTAL),"
                    + "COALESCE(SUM(CASE WHEN payments.payment = 'debt' THEN payments.TOTAL ELSE 0 END),0),"
                    + "COALESCE(SUM(CASE WHEN payments.payment = 'debtpaid' THEN payments.TOTAL ELSE 0 END),0) " +
        ...
        if (valtickets == null) {
            ...
            p.m_dPaymentsDebt = (Double) valtickets[2];
            p.m_dPaymentsDebtPaid = (Double) valtickets[3];
...
//        return Formats.TIMESTAMP.formatValue(m_dDateEnd);
    ...
    public String printPaymentsTotalWD() {
        return Formats.CURRENCY.formatValue(m_dPaymentsTotal-m_dPaymentsDebt);
    }
    public String printPaymentsTotalWDP() {
        return Formats.CURRENCY.formatValue(m_dPaymentsTotal-m_dPaymentsDebtPaid);
    }
     public String printPaymentsTotalWDDP() {
        return Formats.CURRENCY.formatValue(m_dPaymentsTotal-m_dPaymentsDebt-m_dPaymentsDebtPaid);
    }
      public String printPaymentsTotalD() {
        return Formats.CURRENCY.formatValue(m_dPaymentsDebt);
    }
    public String printPaymentsTotalDP() {
        return Formats.CURRENCY.formatValue(m_dPaymentsDebtPaid);
    }
    /**
     *
     * @return
