...
public class CustomersPayment extends javax.swing.JPanel implements JPanelView, BeanFactoryApp {
    ...
    private DataLogicFiscal m_dlFiscal = null;
    ...
    public void init(AppView app) throws BeanFactoryException {
        ...
        m_dlFiscal = (DataLogicFiscal) app.getBean("com.ghintech.fiscalprint.DataLogicFiscal");
    }
...
//        btnPay.setEnabled(customer.getCurdebt() != null && customer.getCurdebt().doubleValue() > 0.0);
    ...
    private void printTicket(String resname, TicketInfo ticket, CustomerInfoExt customer) {
        m_ticket =ticket;
        Properties propsdb = dlsystem.getResourceAsProperties(m_dlFiscal.getHostName() + "/properties");
        try {
            taxeslogic = new TaxesLogic(dlsales.getTaxList().list());
        } catch (BasicException ex) {
        }   
        
        if(dlsystem.getResourceAsText("ticket.customerpayment").equals("")){
            ...
            try{
                ...
                this.evalScript(dlsystem.getResourceAsText("ticket.customerpayment"));
    ...
    public Object evalScript(String code, CustomersPayment.ScriptArg... args) throws ScriptException {
            
        ScriptEngine script = ScriptFactory.getScriptEngine(ScriptFactory.BEANSHELL);            

// Mod JG Sept 2011 - Used inside scripts i.e.: Stock Checking
        String sDBUser = app.getProperties().getProperty("db.user");
        String sDBPassword = app.getProperties().getProperty("db.password");
        
        if (sDBUser != null && sDBPassword != null && sDBPassword.startsWith("crypt:")) {
            AltEncrypter cypher = new AltEncrypter("cypherkey" + sDBUser);
            sDBPassword = cypher.decrypt(sDBPassword.substring(6));
        } 
            script.put("hostname", app.getProperties().getProperty("machine.hostname"));
            script.put("dbURL", app.getProperties().getProperty("db.URL")); 
...
// End mod
            ...
            script.put("user", app.getAppUserView().getUser());
            script.put("sales", this);
       

            // more arguments
            for(CustomersPayment.ScriptArg arg : args) {
