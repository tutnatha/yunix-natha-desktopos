...
public class JTicketsBagTicket extends JTicketsBag {
    ...
    private Boolean fiscalinfo=false;
    ...
    public JTicketsBagTicket(AppView app, JPanelTicketEdits panelticket) {
        ...
        m_dlFiscal = (DataLogicFiscal) m_App.getBean("com.ghintech.fiscalprint.DataLogicFiscal");
    ...
    private void readTicket(int iTicketid, int iTickettype) {
        ...
        try {
            ...
            if (ticket == null) {
                ...
                m_ticket = ticket;
                //fiscalinfo = m_dlFiscal.hasfiscalinfo(ticket);
                //m_jEdit.setEnabled(true);
                m_ticketCopy = null;
                //    if(m_ticket.getTicketStatus()> 0) {
                    if(m_ticket.getTicketType()== 1 || m_ticket.getTicketStatus()> 1) {
                        JFrame frame = new JFrame();
                            JOptionPane.showMessageDialog(frame,
        ...
        if (m_ticket != null) {
            if(fiscalinfo){
                try{                
                    ...
                    JMessageDialog.showMessage(this, new MessageInf(MessageInf.SGN_SUCCESS, "Reimprimiendo Ticket Por favor espere"));                
                    this.evalScript(m_dlSystem.getResourceAsText("ticket.print"));
